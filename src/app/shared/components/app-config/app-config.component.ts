...
export class AppConfigComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.sidebarSubscription = this.configService.configActive$.subscribe(
      (value: boolean) => (this.active = value)
    );
  ...
  replaceLink(linkElement: any, theme: string, onComplete: Function) {
    const id = linkElement.getAttribute('id');
    const cloneLinkElement = linkElement.cloneNode(true);

    cloneLinkElement.setAttribute(
      'href',
      linkElement.getAttribute('href').replace(this.config.theme, theme)
    );
